...
export class HeroController {
  ...
  ): Promise<ListResult<HeroSlide>> {
    return this.service.list(clampListQuery(query, Boolean(user)));
  }

  @Get(':id')
  @UseGuards(OptionalJwtAuthGuard)
  findOne(@Param('id') id: string, @CurrentUser() user?: AuthUser): Promise<HeroSlide> {
    return this.service.findOne(id, { withTrashed: Boolean(user) });
  }

  @Post()
  @AdminOnly()
  create(@Body() dto: CreateHeroDto): Promise<HeroSlide> {
  ...
  @Put(':id')
  @AdminOnly()
  update(@Param('id') id: string, @Body() dto: UpdateHeroDto): Promise<HeroSlide> {
    return this.service.update(id, { ...dto });
  }

  @Patch(':id')
  @AdminOnly()
  patch(@Param('id') id: string, @Body() dto: UpdateHeroDto): Promise<HeroSlide> {
    return this.service.update(id, { ...dto });
  }

  @Patch(':id/visibility')
  @AdminOnly()
  setVisibility(@Param('id') id: string, @Body() dto: VisibilityDto): Promise<HeroSlide> {
  ...
  @Post(':id/restore')
  @AdminOnly()
  restore(@Param('id') id: string): Promise<HeroSlide> {
