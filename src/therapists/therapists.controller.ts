...
export class TherapistsController {
  ...
  ): Promise<ListResult<Therapist>> {
    return this.service.list(clampListQuery(query, Boolean(user)));
  }

  @Get(':id')
  @UseGuards(OptionalJwtAuthGuard)
  findOne(@Param('id') id: string, @CurrentUser() user?: AuthUser): Promise<Therapist> {
    return this.service.findOne(id, { withTrashed: Boolean(user) });
  }

  @Post()
  @AdminOnly()
  create(@Body() dto: CreateTherapistDto): Promise<Therapist> {
  ...
  @Put(':id')
  @AdminOnly()
  update(@Param('id') id: string, @Body() dto: UpdateTherapistDto): Promise<Therapist> {
    return this.service.update(id, { ...dto });
  }

  @Patch(':id')
  @AdminOnly()
  patch(@Param('id') id: string, @Body() dto: UpdateTherapistDto): Promise<Therapist> {
    return this.service.update(id, { ...dto });
  }

  @Patch(':id/visibility')
  @AdminOnly()
  setVisibility(@Param('id') id: string, @Body() dto: VisibilityDto): Promise<Therapist> {
  ...
  @Post(':id/restore')
  @AdminOnly()
  restore(@Param('id') id: string): Promise<Therapist> {
