...
export class NoticesController {
  ...
  ): Promise<ListResult<Notice>> {
    return this.service.list(clampListQuery(query, Boolean(user)));
  }

  @Get(':id')
  @UseGuards(OptionalJwtAuthGuard)
  findOne(@Param('id') id: string, @CurrentUser() user?: AuthUser): Promise<Notice> {
    return this.service.findOne(id, { withTrashed: Boolean(user) });
  }

  @Post()
  @AdminOnly()
  create(@Body() dto: CreateNoticeDto): Promise<Notice> {
  ...
  @Put(':id')
  @AdminOnly()
  update(@Param('id') id: string, @Body() dto: UpdateNoticeDto): Promise<Notice> {
    return this.service.update(id, { ...dto });
  }

  @Patch(':id')
  @AdminOnly()
  patch(@Param('id') id: string, @Body() dto: UpdateNoticeDto): Promise<Notice> {
    return this.service.update(id, { ...dto });
  }

  @Patch(':id/visibility')
  @AdminOnly()
  setVisibility(@Param('id') id: string, @Body() dto: VisibilityDto): Promise<Notice> {
  ...
  @Post(':id/restore')
  @AdminOnly()
  restore(@Param('id') id: string): Promise<Notice> {
