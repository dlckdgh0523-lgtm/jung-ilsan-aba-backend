...
## 7. (2단계) 알림톡 승인 상태 표시 — 2026-09-14 추가

블로그 초안이 생기면 백엔드가 대표 카카오톡으로 알림톡을 보내고, 대표는 폰에서
공개/보류를 결정한다(관리자페이지 없이). 관리자페이지에서는 그 상태를 보여주면 된다.

- 게시글 목록의 블로그(네이버 배지) 행에:
  - **알림 상태**: Article에 review 정보를 별도 API 없이 표시하려면
    `GET /v1/blog-sync/status`와 무관하게 게시글 조회 응답의 `review` 관계는
    현재 포함되지 않으므로, 간단히는 **[알림 재전송]** 버튼만 제공 —
    `POST /v1/blog-sync/notify/:articleId` → `{ "result": "sent" | "failed" | "disabled" }`
    - `sent` → "알림톡 전송됨" 토스트
    - `failed` → "전송 실패 — 알림톡 설정/잔액 확인" 경고
    - `disabled` → "알림톡이 꺼져 있습니다(ALIMTALK_ENABLED)"
    - 409 = 이미 공개/보류가 결정된 글
  - 재전송하면 **이전 링크는 무효**가 된다는 확인 문구 권장.
- **알림톡 연동 상태 카드** (설정 또는 게시글 관리 상단):
  - `GET /v1/alimtalk/status` → `{ enabled, provider, recipients(마스킹), templateReview, fallbackSms, lastSend }`
  - `POST /v1/alimtalk/test` body `{ "to"?: "01012345678" }` → 테스트 발송 버튼
- 리뷰 결정 내역(공개/보류, 시각, kakao-link/admin)은 백엔드 `article_reviews` 테이블에
  쌓인다. 목록 노출이 필요해지면 백엔드에 조회 API 추가 요청할 것(현재 미노출).
